...
RSpec.describe Cron::Datagouv::FileDeposeByMonthJob, type: :job do
  FileDeposeByMonthJob = Cron::Datagouv::FileDeposeByMonthJob

  def format(date) = date.strftime(FileDeposeByMonthJob::DATE_FORMAT)
  ...
  describe 'perform' do
    ...
    subject(:sent_csv) do
      ...
      FileDeposeByMonthJob.perform_now
    ...
    context 'when there is no existing csv' do
      ...
      it 'sends a csv with one row from the previous month' do
        ...
        expect(sent_csv.first['nb_dossiers_deposes_par_mois']).to eq(0)
      end
    end
    ...
    context 'when 2 months are missing' do
      ...
      let(:existing_csv) do
        csv = CSV::Table.new([], headers: FileDeposeByMonthJob::HEADERS)
      ...
      it 'sends a csv with one row from the previous month' do
        expected_csv = [
          ["mois", "nb_dossiers_deposes_par_mois"],
          ["2023-10", 10],
          ["2023-11", 11],
          ["2023-12", 0],
          ["2024-01", 0]
  ...
  describe 'data_for' do
    ...
    subject { FileDeposeByMonthJob.new.send(:data_for, month:) }
