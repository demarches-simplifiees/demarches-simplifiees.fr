...
describe Cron::LLMEnqueueNightlyImproveProcedureJob, type: :job do
  ...
  it 'enqueues the dedicated job only for procedures with feature enabled' do
    ...
    expect(LLM::ImproveProcedureJob).to have_been_enqueued.with(p1, LLMRuleSuggestion.rules.fetch(:improve_label), action: "nightly")
    expect(LLM::ImproveProcedureJob).not_to have_been_enqueued.with(p2, LLMRuleSuggestion.rules.fetch(:improve_label), action: "nightly")
  end
end
