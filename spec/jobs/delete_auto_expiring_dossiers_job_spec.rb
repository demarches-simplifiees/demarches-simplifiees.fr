...
RSpec.describe DeleteAutoExpiringDossiersJob, type: :job do
  ...
  context "Suppression automatique des dossiers : " do
    ...
    it 'verification de la creation de mail' do
      ...
      expect(DossierMailer).to have_received(:notify_near_deletion).with(["#{nouveau_brouillon_dossier2.id} qui concerne la procedure '#{procedure.libelle}', doit être déposé avant le #{Date.today + 1.month} 00:00:00 +0200, sinon il sera supprimé"], nouveau_brouillon_dossier2.user.email)
      expect(DossierMailer).to have_received(:notify_near_deletion).with(["#{nouveau_construction_dossier2.id} qui concerne la procedure '#{procedure.libelle}', doit être traité avant le #{Date.today + 1.month} 00:00:00 +0200", "#{nouveau_instruction_dossier2.id} qui concerne la procedure '#{procedure.libelle}', doit être traité avant le #{Date.today + 1.month} 00:00:00 +0200"], procedure.administrateurs.pluck(:email)[0])
    end
  end
