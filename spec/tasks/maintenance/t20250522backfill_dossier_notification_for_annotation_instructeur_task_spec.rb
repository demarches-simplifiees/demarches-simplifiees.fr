...
module Maintenance
  RSpec.describe T20250522backfillDossierNotificationForAnnotationInstructeurTask do
    describe "#collection" do
      subject(:collection) { described_class.collection }

      let(:instructeur) { create(:instructeur) }
      let!(:dossier_1) { create(:dossier, last_champ_private_updated_at: 2.days.ago) }
      let!(:follow_1) { create(:follow, dossier: dossier_1, instructeur:, annotations_privees_seen_at: 3.days.ago) }
      let!(:dossier_2) { create(:dossier, last_champ_private_updated_at: 2.days.ago) }
      let!(:follow_2) { create(:follow, dossier: dossier_2, instructeur:, annotations_privees_seen_at: 1.day.ago) }
