  ...
  describe '.refresh_notifications' do
    ...
    context "when instructeur changes a preference from 'none' to 'all' " do
      ...
      it "creates notifications of notification_type in question on all dossiers" do
        ...
        expect(DossierNotification.pluck(:instructeur_id).uniq).to eq([instructeur.id])
        expect(DossierNotification.pluck(:notification_type).uniq).to eq(['dossier_modifie'])
        expect(DossierNotification.pluck(:dossier_id)).to match_array([dossier_followed_to_notify.id, dossier_not_followed_to_notify.id])
      end
    end
