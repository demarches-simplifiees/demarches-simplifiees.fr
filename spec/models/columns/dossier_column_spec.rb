  ...
  describe '#filtered_ids' do
    ...
    context 'for a dossier state column' do
      ...
      context 'when searching for multiple states' do
        let!(:dossier_pending_correction) { create(:dossier, :en_construction, procedure:) }
        let!(:dossier_correction) { create(:dossier_correction, dossier: dossier_pending_correction) }

        let(:search_terms) { { operator: 'match', value: ["en_construction", "pending_correction", "accepte"] } }

        it { is_expected.to contain_exactly(dossier_en_construction.id, dossier_pending_correction.id, dossier_accepte.id) }
      end
    end
  end
end
