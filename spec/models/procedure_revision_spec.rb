describe ProcedureRevision do
  ...
  describe 'compare_transitions_rules' do
    include Logic
    let(:new_draft) { procedure.create_new_revision }
    subject { procedure.active_revision.compare_transitions_rules(new_draft.reload) }

    context 'when transitions_rules changes' do
      ...
      context 'when transitions_rules added' do
        before do
          new_draft.update!(transitions_rules: ds_eq(champ_value(yes_no_tdc.stable_id), constant(true)))
        end

        it { is_expected.to include(an_instance_of(ProcedureRevisionChange::AddTransitionsRule)) }
      end

      context 'when transitions_rules removed' do
        before do
          procedure.published_revision.update!(transitions_rules: ds_eq(champ_value(yes_no_tdc.stable_id), constant(true)))
        end

        it { is_expected.to include(an_instance_of(ProcedureRevisionChange::RemoveTransitionsRule)) }
      end

      context 'when transitions_rules changed' do
        before do
          procedure.published_revision.update!(transitions_rules: ds_eq(champ_value(yes_no_tdc.stable_id), constant(true)))
          new_draft.update!(transitions_rules: ds_and([
            ds_eq(champ_value(yes_no_tdc.stable_id), constant(true)),
            empty_operator(empty, empty)
          ]))
        end

        it { is_expected.to include(an_instance_of(ProcedureRevisionChange::ChangeTransitionsRule)) }
      end
    end
  end
