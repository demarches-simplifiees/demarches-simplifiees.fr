...
describe Dossier do
  ...
  describe '#send_brouillon_expiration_notices' do
    ...
    context "Envoi de message pour les dossiers expirant dans - d'un mois" do
      ...
      it 'verification de la creation de mail' do
        expect(DossierMailer).to have_received(:notify_near_deletion).with(brouillon_close_to_expiration.user, [brouillon_close_to_expiration])
        expect(DossierMailer).to have_received(:notify_near_deletion).with(expired_brouillon.user, [expired_brouillon])
      end
  ...
  describe '#destroy_brouillons_and_notify' do
    ...
    it 'notifies deletion' do
      ...
      expect(DossierMailer).to have_received(:notify_deletion).with(expired_brouillon.user, [expired_brouillon.hash_for_deletion_mail])
    end

    it 'deletes the expired brouillon' do
      expect { DeleteDossier.find_by(dossier_id: expired_brouillon.id).to be_present }
