...
RSpec.describe DossierNotification, type: :model do
  ...
  describe '.notifications_for' do
    ...
    context 'a given instructeur and one dossier' do
      ...
      it { is_expected.to include(notification_instructeur) }
      it { is_expected.to include(notification_grp_instructeur) }
      it { is_expected.not_to include(other_notification_instructeur) }
      it { is_expected.not_to include(other_notification_grp_instructeur) }
    end
    ...
    context 'a given instructeur and a list of dossiers' do
      ...
      it { expect(subject[dossier.id]).to include(notification_instructeur) }
      it { expect(subject[dossier.id]).to include(notification_grp_instructeur) }
      it { expect(subject[other_dossier.id]).to include(other_notification_instructeur) }
      it { expect(subject[other_dossier.id]).to include(other_notification_grp_instructeur) }
    end
    ...
    context 'a given instructeur on a procedure' do
      ...
      it { expect(subject['traites'][dossier.id]).to include(notification_instructeur) }
      it { expect(subject['traites'][dossier.id]).to include(notification_grp_instructeur) }
      it { expect(subject['a-suivre'][other_dossier.id]).to include(other_notification_instructeur) }
      it { expect(subject['a-suivre'][other_dossier.id]).to include(other_notification_grp_instructeur) }
    end
  end
end
