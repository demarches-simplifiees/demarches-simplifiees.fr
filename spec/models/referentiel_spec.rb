    ...
    describe "referentiel" do
      ...
      describe 'APIReferentiel' do
        it 'validates presentater as exact_match/autocomplete or nil' do
          expect(build(:referentiel_api, mode: 'exact_match').tap(&:validate).errors.map(&:attribute)).not_to include(:mode)
          expect(build(:referentiel_api, mode: 'autocomplete').tap(&:validate).errors.map(&:attribute)).not_to include(:mode)
          expect(build(:referentiel_api, mode: nil).tap(&:validate).errors.map(&:attribute)).not_to include(:mode)
          expect(build(:referentiel_api, mode: 'wrong').tap(&:validate).errors.map(&:attribute)).to include(:mode)
        end
