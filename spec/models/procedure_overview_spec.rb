...
describe ProcedureOverview, type: :model do
  ...
  let(:procedure_overview) { ProcedureOverview.new(procedure, procedure.dossiers) }

  describe 'dossiers_en_instruction_count' do
    let!(:en_instruction_dossier) do
      create(:dossier, procedure: procedure, state: Dossier.states.fetch(:en_instruction), created_at: 1.day.ago)
    end
  ...
  describe 'old_dossiers_en_instruction' do
    let!(:old_dossier_en_instruction) do
      create(:dossier, procedure: procedure, state: Dossier.states.fetch(:en_instruction), en_instruction_at: 1.month.ago)
    end

    let!(:dossier_en_instruction) do
      create(:dossier, procedure: procedure, state: Dossier.states.fetch(:en_instruction), en_instruction_at: 1.day.ago)
    end
  ...
  describe 'dossiers_en_construction_count' do
    let!(:dossier_en_construction) do
      create(:dossier, procedure: procedure, state: Dossier.states.fetch(:en_construction), created_at: 1.day.ago)
    end
  ...
  describe 'created_dossiers_count' do
    let!(:created_dossier_during_the_week) do
      create(:dossier, procedure: procedure, created_at: 1.day.ago, state: Dossier.states.fetch(:en_instruction))
    end

    let!(:created_dossier_before_the_week) do
      create(:dossier, procedure: procedure, created_at: 1.month.ago, state: Dossier.states.fetch(:en_instruction))
    end
