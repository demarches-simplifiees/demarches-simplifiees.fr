...
describe ProcedureOverview, type: :model do
  ...
  describe 'dossiers_en_construction_count' do
    let!(:dossier_en_construction) do
      create(:dossier, procedure: procedure, state: Dossier.states.fetch(:en_construction), depose_at: 1.day.ago)
    end
  ...
  describe 'created_dossiers_count' do
    let!(:created_dossier_during_the_week) do
      create(:dossier, procedure: procedure, created_at: 1.day.ago, state: Dossier.states.fetch(:en_construction), depose_at: 1.day.ago)
    end

    let!(:created_dossier_before_the_week) do
      create(:dossier, procedure: procedure, created_at: 1.month.ago, state: Dossier.states.fetch(:en_construction), depose_at: 1.month.ago)
    end
