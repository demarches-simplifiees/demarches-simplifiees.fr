...
describe Administrateurs::AttestationTemplateV2sController, type: :controller do
  ...
  describe 'GET #show' do
    subject do
      get :show, params: { procedure_id: procedure.id, format:, kind: :acceptation }
      response.body
    end
  ...
  describe 'GET edit' do
    ...
    context 'acceptation kind' do
      let(:params) { { procedure_id: procedure.id, kind: :acceptation } }
      context 'if an attestation template does not exists yet on the procedure' do
      ...
      context 'attestation template published exist without draft' do
        let(:attestation_template) { build(:attestation_template, :v2, :published, kind: :acceptation) }
      ...
      context 'attestation template draft already exist on v2' do
        let(:attestation_template) { build(:attestation_template, :v2, :draft, kind: :acceptation) }
  ...
  describe 'POST create' do
    ...
    subject do
      post :create, params: { procedure_id: procedure.id, attestation_template: update_params, kind: }, format: :turbo_stream
      response.body
    end

    context 'acceptation kind' do
      let(:kind) { :acceptation }
  ...
  describe 'PATCH update' do
    render_views
    subject do
      patch :update, params: { procedure_id: procedure.id, attestation_template: update_params, kind: :acceptation }, format: :turbo_stream
      response.body
    end
  ...
  describe 'POST reset' do
    ...
    subject do
      patch :reset, params: { procedure_id: procedure.id, kind: :acceptation }
      response.body
    end

    it "delete draft, keep published" do
      expect(procedure.attestation_templates.count).to eq(2)
      expect(subject).to redirect_to(edit_admin_procedure_attestation_template_v2_path(procedure, kind: :acceptation))
