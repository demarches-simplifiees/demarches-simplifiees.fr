    ...
    context 'when the instructeur has chosen not to have certain notifications' do
      ...
      it {
        expect(react_props_items["items"].map(&:first)).to eq([
          "Déposé depuis longtemps", "Expire bientôt", "Bientôt supprimé", "Dossier modifié", "Annotation privée", "Avis externe", "En attente de correction", "En attente d'avis externe",
        ])
      }
    end
  end
