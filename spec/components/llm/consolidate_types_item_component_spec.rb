...
RSpec.describe LLM::ConsolidateTypesItemComponent, type: :component do
  ...
  let(:llm_rule_suggestion) { create(:llm_rule_suggestion, procedure_revision: revision, rule: 'consolidate_types', schema_hash:, state: 'completed') }

  describe '.step_title' do
    it 'returns the correct title' do
      expect(described_class.step_title).to eq("Consolidation des types de champs")
    end
  end
  ...
  describe 'rendering via SuggestionFormComponent' do
    ...
    context 'with type change update' do
      ...
      it 'renders the type transition and field label' do
        ...
        expect(subject).to have_css('.fr-badge', text: /Texte court|Text/)
        expect(subject).to have_css('.fr-badge.fr-badge--green-emeraude', text: /Adresse électronique|Email/)
        # Check that arrow icon is present
    ...
    context 'with destroy operation' do
      before do
        create(:llm_rule_suggestion_item,
          ...
          justification: 'Champ redondant avec le type address')
      end

      it 'renders the destroy badge and field name' do
        expect(subject).to have_text("Suppression")
        expect(subject).to have_text("Email du contact")
        expect(subject).to have_text("Champ redondant avec le type address")
      end
    end
  end
end
