...
describe Dossiers::ReferentielDisplayUsagerComponent, type: :component do
  ...
  describe 'renders items' do
    before { champ.update_column(:value_json, { display_usager: { "$.string" => "string", "$.integer" => 42 } }) }
    ...
    describe 'renders date and datetime items' do
      before do
        champ.update_column(:value_json, { display_usager: { "$.date" => "14/06/2024", "$.datetime" => "2024-06-19T15:30" } })
      end
      let(:referentiel_mapping) do
        {
    ...
    describe 'renders boolean items' do
      before do
        champ.update_column(:value_json, { display_usager: { "$.bool_true" => true, "$.bool_false" => false } })
      end
      let(:referentiel_mapping) do
        {
    ...
    describe 'renders list items' do
      before do
        champ.update_column(:value_json, { display_usager: { "$.list" => ["Option 1", nil, "Option 3"] } })
      end
      let(:referentiel_mapping) do
        {
    ...
    describe 'renders XSS safely' do
      before do
        champ.update_column(:value_json, { display_usager: { "$.xss" => '<script>alert("xss")</script>' } })
      end
      let(:referentiel_mapping) do
        {
    ...
    describe 'does not renders object values safely' do
      before do
        champ.update_column(:value_json, { display_usager: { "$.object" => { foo: "bar", baz: 42 } } })
      end
      let(:referentiel_mapping) do
        {
      ...
      it 'skips all' do
        expect(subject).not_to have_content("Champ objet")
        expect(subject).not_to have_content('{"foo":"bar","baz":42}')
      end
    end
  end
end
