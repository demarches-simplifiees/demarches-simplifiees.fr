...
feature 'Using Paraphe field', js: true do
  ...
  let!(:procedure) { create(:procedure, :published, :with_paraphe, instructeurs: [instructeur1, instructeur2, instructeur3]) }
  ...
  scenario 'instructor1 cannot validate paraphe', :js do
    ...
    expect(page).to have_field('paraphe_to_test', disabled: true)
  end
  ...
  scenario 'instructor1 cannot validate paraphe', :js do
    ...
    expect(page).to have_field('paraphe_to_test', disabled: false)
    check('paraphe_to_test')
    expect(page).to have_current_path(annotations_privees_instructeur_dossier_path(dossier.procedure, dossier))
    expect(page).to have_field('paraphe_to_test', checked: true)

    check_fields_before_paraphe_are_disabled
    check_fields_after_paraphe_are_enabled
  ...
  def check_fields_before_paraphe_are_disabled
    check_fields('preceding', true)
  end

  def check_fields_after_paraphe_are_enabled
    check_fields('following', false)
  end

  def check_fields(axe, disabled)
    paraphe_label_path = ".//label[normalize-space(text())='paraphe_to_test']"
    divs_path = paraphe_label_path + "/parent::div/#{axe}-sibling::div"
