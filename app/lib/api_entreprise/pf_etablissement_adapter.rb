class APIEntreprise::PfEtablissementAdapter < APIEntreprise::Adapter
  ...
  def process_params
    no_etablissement = @siret.to_s[6..9]
    etablissements = data_source.sort_by { |a| a[:numEtablissement] }.filter { |h| h[:dateRadiation].nil? }
    etablissement = { siege_social: true }
    if no_etablissement.present?
      no_etablissement = no_etablissement.to_i
      etablissements = [etablissements.find { |e| e[:numEtablissement] == no_etablissement }].compact
    end
    ...
    [etablissement, etablissements.size > 1 ? etablissements : nil]
  end
