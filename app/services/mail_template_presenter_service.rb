class MailTemplatePresenterService
  ...
  def self.create_commentaire_for_state(dossier, state)
    if dossier.procedure.accuse_reception? && Dossier::TERMINE.include?(state)
      CommentaireService.create!(CONTACT_EMAIL, dossier, body: I18n.t('layouts.mailers.accuse_reception.commentaire_html', service: dossier.procedure.service.nom))
    else
