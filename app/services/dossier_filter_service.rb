...
class DossierFilterService
  ...
  def self.sorted_ids(dossiers, sorted_column, instructeur, count)
    ...
    when 'notifications'
      ...
      if order == 'desc'
        dossiers_id_with_notifications_v2 + (dossiers_id_with_notifications - dossiers_id_with_notifications_v2) +
            (dossiers.order('dossiers.updated_at desc').ids - dossiers_id_with_notifications - dossiers_id_with_notifications_v2)
      else
