...
module LLM
  class BaseImprover
    ...
    def generate_for(suggestion, action: nil, user_id: nil)
      calls = run_tool_call(suggestion, action:, user_id:)
      calls
    ...
    def build_messages(suggestion)
      safe_schema = sanitize_schema_for_prompt(suggestion.procedure_revision.schema_to_llm)
    ...
    def run_tool_call(suggestion, procedure_id: nil, rule: nil, action: nil, user_id: nil)
