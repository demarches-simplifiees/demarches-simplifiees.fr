class Procedure < ApplicationRecord
  ...
  def existing_procedure_path(path)
    if procedure_path.present?
      ProcedurePath.where.not(id: procedure_path.id).find_by(path: path)
    else
      ProcedurePath.find_by(path: path)
    end
  end
  ...
  def publish_test!(path)
    update!({ test_started_at: Time.now, archived_at: nil })
    create_procedure_path!(path: path, administrateur: administrateur)
  end
