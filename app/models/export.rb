class Export < ApplicationRecord
  ...
  def self.find_or_create_fresh_export(format, groupe_instructeurs, instructeur, time_span_type: time_span_types.fetch(:everything), statut: statuts.fetch(:tous), procedure_presentation: nil, export_template: nil)
    attributes = {
      format:,
      export_template:,
      time_span_type:,
  ...
  def blob
    service = ProcedureExportService.new(procedure, dossiers_for_export, export_template)
