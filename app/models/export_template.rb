class ExportTemplate < ApplicationRecord
  ...
  def paths=(paths)
    ...
    paths.compact_blank.each do |full_path|
      ...
      columns << current_column.merge(libelle:)
    end
  ...
  def all_usager_paths
    all_usager_columns.map do
      Path.new(full_path: "dossier_#{_1.to_s}", libelle: columns_meta[_1][:libelle])
    end
  end
  ...
  def tdc_columns
    columns.select{ _1['source'] == 'tdc' }
  end

  def columns_without_repet
    columns.select{ _1['source'] != 'repet' }
  end
  ...
  def columns_meta
    {
      ...
      etablissement_code_insee_localite: { libelle: 'Établissement code INSEE localité', get_value: ->(d) { etablissement&.code_insee_localite } },
...
#dead code ?
  def columns_without_repetition
    content["columns"].select{ _1['source'] != 'repet' }
  end
