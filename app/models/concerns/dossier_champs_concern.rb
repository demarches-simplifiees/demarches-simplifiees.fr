...
module DossierChampsConcern
  ...
  def repetition_row_ids(type_de_champ)
    ...
    return @repetition_row_ids.fetch(type_de_champ.stable_id) if @repetition_row_ids&.key?(type_de_champ.stable_id)
    ...
    @repetition_row_ids ||= {}
    @repetition_row_ids[type_de_champ.stable_id] = row_ids.uniq.sort
  end
  ...
  def stable_id_in_revision?(stable_id)
    revision_stable_ids.member?(stable_id)
  end
