...
module DossierTreeConcern
  ...
  def public_tree(profile: nil)
    DossierTree.new(coordinates: public_coordinates, procedure:, dossier: self, stream:, profile:)
  end

  def private_tree(profile: nil)
    DossierTree.new(coordinates: private_coordinates, procedure:, dossier: self, stream:, profile:)
  end
