module ProcedureCloneConcern
  ...
  def clone(options, admin)
    populate_champ_stable_ids

    procedure = self.deep_clone(include: clone_associations(options, admin)) do |original, kopy|
      ClonePiecesJustificativesService.clone_attachments(original, kopy)
    end

    procedure = set_default_values(procedure, admin)

    procedure = set_custom_values(procedure, options, admin)
  ...
  def set_default_values(procedure, admin)
  ...
  def set_custom_values(procedure, options, admin)
  ...
  def clone_associations(options, admin)
    associations = {
      draft_revision: {
