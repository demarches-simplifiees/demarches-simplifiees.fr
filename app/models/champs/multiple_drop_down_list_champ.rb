...
class Champs::MultipleDropDownListChamp < Champ
  ...
  def referentiels_from(value)
    ...
    referentiels = {}
    values.each do |id|
      referentiel_item = type_de_champ.referentiel.items.find_by(id: id.to_i)
      ...
      referentiels = referentiels.merge(id => { data: referentiel_item.data.merge(headers:) })
    end
    referentiels
  end
  ...
  def values_are_in_options
    ...
    return if (json - referentiels.keys).empty? && drop_down_advanced?
