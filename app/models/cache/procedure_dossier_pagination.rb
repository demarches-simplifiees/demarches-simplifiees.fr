...
class Cache::ProcedureDossierPagination
  ...
  def write_cache(value) = cache.value = value

  def read_cache = Hash(cache.value).with_indifferent_access

  def ids = read_cache[:ids]

  def refresh_cache_after?(from_id:) = from_id.in?(ids.last(TRESHOLD_BEFORE_REFRECH))

  def refresh_cache_before?(from_id:) = from_id.in?(ids.first(TRESHOLD_BEFORE_REFRECH))
