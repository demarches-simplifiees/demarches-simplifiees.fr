class Dossier < ApplicationRecord
  ...
  has_many :champs_to_destroy, class_name: 'Champ', inverse_of: false, dependent: :destroy
  ...
  def build_default_champs
    self.champs = revision.types_de_champ.filter_map do |type_de_champ|
      if type_de_champ.repetition?
        if type_de_champ.mandatory? || type_de_champ.private?
          type_de_champ.champ.build(dossier: self, row_id: ULID.generate)
        end
      elsif !type_de_champ.non_fillable?
        type_de_champ.champ.build(dossier: self)
      end
    end
  end
