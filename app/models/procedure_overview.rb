...
class ProcedureOverview
  ...
  def initialize(procedure, dossiers)
    ...
    @old_dossiers_en_instruction =
      @dossiers_en_instruction.filter { |d| d.en_instruction_at && d.en_instruction_at < 1.week.ago }

    @old_dossiers_en_construction =
      @dossiers_en_construction.filter { |d| d.depose_at && d.depose_at < 1.week.ago }
