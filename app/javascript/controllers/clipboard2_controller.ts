...
export class Clipboard2Controller extends Controller {
  ...
  private setupChampHoverListeners(): void {
    [...this.element.querySelectorAll<HTMLDivElement>('.copy-btn')]
      .filter((wrapper) => wrapper.innerText?.trim() !== '')
      .forEach((wrapper) => {
  ...
  private copyContent(wrapper: HTMLDivElement): void {
    this.#copySpan.remove();
  ...
  private showCopiedSpan(wrapper: HTMLDivElement): void {
  ...
  private insertSpan(wrapper: HTMLDivElement, span: HTMLElement): void {
