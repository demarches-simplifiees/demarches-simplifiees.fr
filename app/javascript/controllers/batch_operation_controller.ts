...
export class BatchOperationController extends ApplicationController {
  ...
  injectSelectedIdsIntoModal(event: Event) {
    ...
    modalForm.querySelectorAll('input[name="batch_operation[dossier_ids][]"]').forEach((el) => el.remove());

    const hiddenInput = document.querySelector<HTMLInputElement>('#input_multiple_ids_batch_operation');
    let ids: string[] = [];

    if (hiddenInput && hiddenInput.value.trim() !== '') {
      // Cas 1 : sélection étendue (select all + select more)
      ids = hiddenInput.value.split(',').map((id) => id.trim()).filter(Boolean);
    } else {
    ...
    const confidentialRadio = document.querySelector<HTMLInputElement>('#confidentiel_true');
