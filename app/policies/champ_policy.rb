class ChampPolicy < ApplicationPolicy
  class WriteScope < ApplicationScope
    def resolve
      if user.blank?
        return scope.none
      end
      ...
      if instructeur.present?
        # Additionnaly, instructeurs can write to private champs
  ...
  class ReadScope < ApplicationScope
    def resolve
      if user.blank?
        return scope.none
      end
      ...
      if instructeur.present?
        # Additionnaly, instructeurs can read public and private champs
        # on dossiers they are allowed to instruct.
        instructeur_clause = joined_scope
          .where('instructeurs.id': instructeur.id)
        resolved_scope = resolved_scope.or(instructeur_clause)
      end
