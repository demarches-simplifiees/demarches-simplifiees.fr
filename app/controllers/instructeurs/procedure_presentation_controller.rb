...
module Instructeurs
  class ProcedurePresentationController < InstructeurController
    ...
    def add_filter
      ...
      new_filter = FilteredColumn.new(column:, filter:)
    ...
    def refresh_column_filter
      @column = ColumnType.new.cast(params['column_id'])
    ...
    def filtered_column_from_params
      @filtered_column_from_params ||= FilteredColumn.new(column: ColumnType.new.cast(params[:column_id]), filter: params[:filter])
    end
    ...
    def filter_params
      params.permit(:column_id, :statut, filter: [:operator, :value, values: []])
    end
