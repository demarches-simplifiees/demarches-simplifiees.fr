class KeycloakController < ApplicationController
  ...
  def login
    redirect_to KeycloakService.authorization_uri
  end

  def callback
    fetched_fci = KeycloakService.retrieve_user_informations_particulier(params[:code])
    ...
    connect_keycloak(fci.user)
  rescue Rack::OAuth2::Client::Error => e
    Rails.logger.error e.message
    redirect_keycloak_error_connection
  end
  ...
  def connect_keycloak(user)
    ...
    user.update_attribute('loged_in_with_france_connect', User.loged_in_with_france_connects.fetch(:keycloak))

    redirect_to stored_location_for(current_user) || root_path(current_user)
  end

  def redirect_keycloak_error_connection
    flash.alert = t('errors.messages.keycloak.connexion')
    redirect_to(new_user_session_path)
  end
end
