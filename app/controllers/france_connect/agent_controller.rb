class FranceConnect::AgentController < ApplicationController
  before_action :redirect_to_login_if_fc_aborted, only: [:callback]

  def login
    redirect_to FranceConnectService.authorization_uri(FRANCE_CONNECT[:agent])
  end

  def callback
    fetched_fci = FranceConnectService.fetch_info(FRANCE_CONNECT[:agent], params[:code])
