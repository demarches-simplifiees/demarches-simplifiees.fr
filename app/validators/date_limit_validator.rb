...
class DateLimitValidator < ActiveModel::Validator
  def validate(record)
    ...
    validate_in_past(record) if record.type_de_champ.date_in_past?
    validate_in_range(record) if record.type_de_champ.range_date?
  end

  private

  def validate_in_past(record)
