...
module Mutations
  class DossierModifierAnnotations < Mutations::BaseMutation
    ...
    def resolve(dossier:, instructeur:, annotations:)
      find_annotations(dossier, annotations)
    end
    ...
    def find_annotations(dossier, annotations)
      annotations, errors = annotations
        .filter_map { find_annotation(dossier, _1) }
        .partition { _1.repetition? || (_1.validate(:champs_private_value) && _1.save) }

      { annotations:, errors: errors.map { _1.errors.full_messages }.presence }
    end
    ...
    def find_annotation(dossier, annotation)
      ...
      return nil if type_de_champ.nil? || !type_de_champ.type_champ.in?(accepted_types)
