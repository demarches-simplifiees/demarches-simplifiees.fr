...
module LLM
  class HeaderComponent < ApplicationComponent
    ...
    def last_suggestion_created_at_tag
      label = if !llm_rule_suggestion.pending?
        timestamp = I18n.l(llm_rule_suggestion.created_at, format: :llm_stepper_last_refresh)
        I18n.t('llm.stepper_component.last_refresh', timestamp:)
      else
        I18n.t('llm.stepper_component.no_suggestion_yet')
      end
      tag.p(
        safe_join([
          tag.span(class: 'fr-icon-time-line fr-btn--icon-left', aria: { hidden: true }),
          label,
        ]),
        class: 'fr-text--bold'
      )
