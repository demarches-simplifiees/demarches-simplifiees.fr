...
class EditableChamp::SectionComponent < ApplicationComponent
  ...
  def initialize(dossier:, nodes: nil, types_de_champ: nil, row_id: nil, row_number: nil, input_labelled_by_prefix: nil)
    nodes ||= to_tree(types_de_champ:)
    @dossier = dossier
    @row_id = row_id
    @row_number = row_number
    @nodes = to_fieldset(nodes:)
    @input_labelled_by_prefix = input_labelled_by_prefix
  end
